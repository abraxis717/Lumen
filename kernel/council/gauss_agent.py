"""
gauss_agent.py — Gauss: Optimization agent (Phase 3.5 Runtime Swarm)

Accepts a MobileModel (GGUF) and a StratifiedRetriever for live LLM-backed
reasoning. Falls back to deterministic optimization when no model is provided.
...
class GaussAgent:
    """Optimization agent with live LLM reasoning.
    ...
    def propose_claims(self, step: int, state: Dict) -> List[GovernedClaim]:
        """Propose an optimization-grounded GovernedClaim via live LLM.
        ...
        if self._model is not None:
            try:
                ...
                logger.warning("Gauss LLM generation failed: %s", exc)
    ...
    def _build_context(self) -> str:
        ...
        try:
            ...
            logger.debug("Gauss context build failed: %s", exc)
            return ""
    ...
    def _format_prompt(step: int, state: Dict, context: str) -> str:
        ...
        prompt = (
            ...
            "You are the Gauss (optimization) agent.\n"
    ...
    def _deterministic_claim(self, step: int) -> List[GovernedClaim]:
        ...
        return [
            GovernedClaim(
                text=f"Optimization: minimum at {3.14159 * (step + 1):.4f}. "
                     f"Gradient converges. Drift: 0.02.",
                confidence=0.83,
                agent=self.name,
