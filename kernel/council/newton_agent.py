"""
newton_agent.py — Newton: Simulation agent (Phase 3.5 Runtime Swarm)

Accepts a MobileModel (GGUF) and a StratifiedRetriever for live LLM-backed
reasoning. Falls back to deterministic simulation when no model is provided.
...
class NewtonAgent:
    """Simulation agent with live LLM reasoning.
    ...
    def propose_claims(self, step: int, state: Dict) -> List[GovernedClaim]:
        """Propose a simulation-grounded GovernedClaim via live LLM.
        ...
        if self._model is not None:
            try:
                ...
                logger.warning("Newton LLM generation failed: %s", exc)
    ...
    def _build_context(self) -> str:
        ...
        try:
            ...
            logger.debug("Newton context build failed: %s", exc)
            return ""
    ...
    def _format_prompt(step: int, state: Dict, context: str) -> str:
        ...
        prompt = (
            ...
            "You are the Newton (simulation) agent.\n"
    ...
    def _deterministic_claim(self, step: int) -> List[GovernedClaim]:
        ...
        return [
            GovernedClaim(
                text=f"Simulation: trajectory stable. Position "
                     f"{step * 0.5:.1f}, velocity {step * 0.1:.1f}.",
                confidence=0.87,
                agent=self.name,
