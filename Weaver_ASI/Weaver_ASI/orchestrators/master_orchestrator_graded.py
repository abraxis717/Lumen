#!/usr/bin/env python3
"""
Weaver ASI — Graded Fault Severity Orchestrator
=================================================
Phase 3: The 4-tier severity ladder (NOMINAL → WARNING → CRITICAL → FATAL).
Each tier has its own action whitelist, healing threshold, and
mitigation escalation.

This is the direct evolution of the original Cathedral-OS graded
orchestrator, now decomposed into the Weaver ASI package architecture.

Run: python3 orchestrators/master_orchestrator_graded.py
"""
import sys
import os
import random
...
class PhysicalWorld:
    ...
    def tick(self):
        ...
        # Stage 3 — pressure spike (tick 10) → FATAL
        if self._tick == 10:
...
def main():
    ...
    from Weaver_ASI.council.adversarial_swarm import StatefulAdversary, RogueOracleAgent
    ...
    newton = NewtonAgent()
    turing = TuringAgent()
    adversary = StatefulAdversary()
    rogue = RogueOracleAgent()
    lumen = LumenAgent(use_http=False)
    ...
    class Conductor:
        ...
        def run_simulation(s, cycles=22):
            print(f"\n{'═'*68}")
            print(f"  Weaver ASI — Graded Fault Severity  ({cycles} cycles)")
            print(f"  Phase 3: 4-Tier Severity Ladder")
            print(f"{'═'*68}")
        ...
        def _summary(s):
            ...
            print(f"  Final gate severity: [{SEVERITY_LABEL[gate.severity]}]")
            if gate.severity == Severity.FATAL:
                print("  ☠ FATAL latch active — Steward action required to clear")
            print(f"{'═'*68}")
            ...
            from Weaver_ASI.lumen.lumen_agent import LumenAgent
            ...
            sync = GraphSync(chronicle=chronicle, instance_name="graded-node-1")
            ...
            foreign_export = {
                "federation_source": "graded-node-2",
                "exported_at": time.time(),
                "exported_by": "graded-node-2",
                "node_count": 1,
            ...
            print(
                f"  [GraphSync] Import from 'graded-node-2': "
            ...
            conflicts = arb.resolve_cross_instance_conflicts(
                graph, conflict_nodes, foreign_source="graded-node-2"
            )
            if conflicts:
            ...
            dc.register_peer("http://graded-node-2:8080")
            ...
            propose_result = dc.propose_to_federation(test_claim, ["http://graded-node-2:8080"])
            print(
